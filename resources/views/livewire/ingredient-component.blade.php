    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <table class="w-full text-left text-sm">
            <thead class="bg-slate-50 text-[10px] font-black uppercase tracking-wider text-slate-400">
                <tr><th class="px-5 py-4">Insumo</th><th class="px-5 py-4">Stock</th><th class="px-5 py-4">Mínimo</th><th class="px-5 py-4 text-right">Acciones</th></tr>
            </thead>
                ...
                @forelse ($ingredients as $ingredient)
                    <tr>
                        ...
                        <td class="px-5 py-4 text-right">
                            <button wire:click="edit({{ $ingredient->id }})" class="px-2 text-amber-600"><i class="fa-solid fa-pen"></i></button>
                            <button wire:click="deleteConfirm({{ $ingredient->id }})" class="px-2 text-rose-600"><i class="fa-solid fa-trash-can"></i></button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="px-5 py-12 text-center text-sm text-slate-400">Aún no hay insumos.</td></tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4">
            <form wire:submit="store" class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
                ...
                <div class="space-y-4">
                    ...
                    <x-input-error :messages="$errors->get('stock')" /><x-input-error :messages="$errors->get('minimum_stock')" />
